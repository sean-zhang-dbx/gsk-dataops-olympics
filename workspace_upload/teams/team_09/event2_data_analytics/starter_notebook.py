...
# MAGIC ### Data
# MAGIC
# MAGIC Step 0 below creates canonical tables in your team catalog:
# MAGIC - `heart_silver_correct` — cleaned patient-level data (~500 rows)
# MAGIC - `heart_gold_correct` — aggregated metrics by age group and diagnosis (~8 rows)
# MAGIC
...
# MAGIC
# MAGIC This creates canonical `heart_silver_correct` and `heart_gold_correct` tables
# MAGIC in your team catalog from the shared reference data.
# MAGIC Everyone starts Event 2 from the same baseline — independent of Event 1 results.

# COMMAND ----------

spark.sql(f"""
    CREATE OR REPLACE TABLE {CATALOG}.default.heart_silver_correct AS
    SELECT * FROM {SHARED_CATALOG}.default.heart_disease
""")
spark.sql(f"COMMENT ON TABLE {CATALOG}.default.heart_silver_correct IS 'Canonical cleaned patient data for Events 2-5'")

spark.sql(f"""
    CREATE OR REPLACE TABLE {CATALOG}.default.heart_gold_correct AS
    SELECT
        CASE
    ...
    GROUP BY 1, 2
""")
spark.sql(f"COMMENT ON TABLE {CATALOG}.default.heart_gold_correct IS 'Canonical aggregated metrics for Events 2-5'")

silver_count = spark.table(f"{CATALOG}.default.heart_silver_correct").count()
gold_count = spark.table(f"{CATALOG}.default.heart_gold_correct").count()
print(f"  heart_silver_correct: {silver_count} rows")
print(f"  heart_gold_correct:   {gold_count} rows")
print(f"  Ready for Event 2!")

# COMMAND ----------

display(spark.table(f"{CATALOG}.default.heart_silver_correct").limit(5))

# COMMAND ----------

display(spark.table(f"{CATALOG}.default.heart_gold_correct"))
...
# MAGIC
# MAGIC Write 3 analytical queries against `heart_silver_correct` to get comfortable with the data.
...
# MAGIC 3. **Add tables:**
# MAGIC    - `{CATALOG}.default.heart_silver_correct`
# MAGIC    - `{CATALOG}.default.heart_gold_correct`
...
# MAGIC
# MAGIC ### heart_silver_correct (patient-level, ~488 rows)
# MAGIC
...
# MAGIC
# MAGIC ### heart_gold_correct (aggregated, ~8 rows)
# MAGIC
# MAGIC | Column | Description |
# MAGIC |--------|------------|
