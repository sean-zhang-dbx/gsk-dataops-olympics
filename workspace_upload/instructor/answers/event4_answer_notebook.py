...
# MAGIC
# MAGIC Teams have `heart_silver_correct` and `heart_gold_correct` from Events 1–3.
...
try:
    ...
    print(f"heart_silver_correct already exists ({silver_count} rows)")
except Exception:
    print("Creating heart_silver_correct from raw data...")
    ...
    raw.write.format("delta").mode("overwrite").saveAsTable(f"{CATALOG}.default.heart_silver_correct")
    print(f"  Created heart_silver_correct: {raw.count()} rows")

try:
    gold_count = spark.table(f"{CATALOG}.default.heart_gold_correct").count()
    print(f"heart_gold_correct already exists ({gold_count} rows)")
except Exception:
    print("Creating heart_gold_correct from heart_silver_correct...")
    spark.sql(f"""
        ...
        ORDER BY 1, 2
    """)
    cnt = spark.table(f"{CATALOG}.default.heart_gold_correct").count()
    print(f"  Created heart_gold_correct: {cnt} rows")
...
serialized_space = json.dumps({
    ...
    "data_sources": {
        "tables": sorted([
            {"identifier": f"{CATALOG}.default.heart_silver_correct",
            ...
            {"identifier": f"{CATALOG}.default.heart_gold_correct",
    ...
    "instructions": {
        "text_instructions": [{
            "id": uuid.uuid4().hex,
            "content": [
                "You are a clinical data analyst. The heart_silver_correct table has patient-level records with: "
                ...
                "heart_gold_correct has aggregated stats by age_group and diagnosis. "
...
{'='*60}
Heart Data:       {CATALOG}.default.heart_silver_correct, heart_gold_correct
