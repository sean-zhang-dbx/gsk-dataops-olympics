...
try:
    ...
    SDK_AVAILABLE = False
    print(f"WARNING: SDK disabled ({type(e).__name__}: {str(e)[:80]})")
...
def score_team(team_name: str) -> dict:
    ...
    # ─── ARTIFACTS (3 pts) ───
    required_tables = ["heart_silver_correct", "heart_gold_correct", "heart_ai_insights", "drug_ai_summary"]
    ...
    if _table_exists(catalog, "executive_briefing"):
        ...
        if cnt > 0:
            try:
                # Try common column names for the briefing text
                eb = spark.table(f"{catalog}.{SCHEMA}.executive_briefing")
                cols = [c.lower() for c in eb.columns]
                if "briefing_text" in cols:
                    briefing = eb.select("briefing_text").collect()[0][0]
                elif "content" in cols:
                    briefing = eb.select("content").collect()[0][0]
                elif "summary" in cols:
                    briefing = eb.select("summary").collect()[0][0]
                else:
                    # Take the longest string column
                    row = eb.limit(1).collect()[0]
                    briefing = max((str(row[c]) for c in eb.columns if row[c]), key=len, default="")
