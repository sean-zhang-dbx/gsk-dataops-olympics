...
# MAGIC %md
# MAGIC ## Step 2.5: Catalog Permission Isolation
# MAGIC
# MAGIC Grants catalog access **per user** (Azure Databricks requires individual user grants,
# MAGIC not workspace group grants, for Unity Catalog).
# MAGIC
# MAGIC **How it works:**
# MAGIC - Each team member gets ALL PRIVILEGES on their team catalog
# MAGIC - Each team member gets read-only access to the shared `dataops_olympics` catalog
# MAGIC - Cross-team access is revoked per user
# MAGIC
# MAGIC **Set `TEAM_MEMBERS` below** with the email addresses assigned to this team.
...
TEAM_MEMBERS = [m.strip() for m in _members_raw.split(",") if m.strip()]

print("=" * 60)
print(f"  PERMISSION ISOLATION — {TEAM_NAME}")
print(f"  Members: {TEAM_MEMBERS or '(none — skipping grants)'}")
print("=" * 60)

for member in TEAM_MEMBERS:
    # Team member owns their team catalog
    spark.sql(f"GRANT ALL PRIVILEGES ON CATALOG {TEAM_NAME} TO `{member}`")
    print(f"  GRANTED ALL PRIVILEGES on {TEAM_NAME} to {member}")

    # Read-only access to shared catalog
    spark.sql(f"GRANT USAGE ON CATALOG {SHARED_CATALOG} TO `{member}`")
    spark.sql(f"GRANT USAGE ON SCHEMA {SHARED_CATALOG}.{SHARED_SCHEMA} TO `{member}`")
    spark.sql(f"GRANT SELECT ON SCHEMA {SHARED_CATALOG}.{SHARED_SCHEMA} TO `{member}`")
    spark.sql(f"GRANT READ VOLUME ON VOLUME {SHARED_CATALOG}.{SHARED_SCHEMA}.{VOLUME_NAME} TO `{member}`")
    print(f"  GRANTED read access on {SHARED_CATALOG} to {member}")

    # Revoke access to other team catalogs
    for other_team in ALL_TEAMS:
        if other_team != TEAM_NAME:
            try:
                spark.sql(f"REVOKE ALL PRIVILEGES ON CATALOG {other_team} FROM `{member}`")
            except Exception:
                pass  # catalog may not exist yet

if not TEAM_MEMBERS:
    print("  No team members specified — catalog created but no user grants applied.")
    print("  Use the assign_teams notebook to grant access later.")
...
candidates = [
    ...
    os.path.join(_nb_dir, "data"),
    "/Workspace/Shared/gsk-dataops-olympics/data",
]
...
try:
    ...
    candidates += [
        ...
        f"/Workspace/Users/{user_email}/gsk-dataops-olympics-instructor/data",
