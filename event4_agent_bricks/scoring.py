...
def _score_uc_functions(catalog: str) -> tuple:
    ...
    try:
        funcs = spark.sql(f"SHOW USER FUNCTIONS IN {catalog}.default").collect()
        func_names = [r.function.split(".")[-1].lower() for r in funcs]
...
teams = [r.team for r in spark.sql(f"SELECT team FROM {_RT}").collect()]
if not teams:
    teams = ["answer_key"]
print(f"Scoring teams: {teams}")
...
for r in results:
    t = r["team"]
    if spark.sql(f"SELECT 1 FROM {_RT} WHERE team = '{t}'").count() == 0:
        spark.sql(f"INSERT INTO {_RT} VALUES ('{t}')")
    ...
    print(f"  {t}: {r['total']} pts written to leaderboard")
